...
// Validate academite functions are available looping through fuction object, throwing an error message if function is broken
...
function isValidColumn(columnArray) {
  return (
    ...
    columnArray.every(function (item) {
      return ["nought", "cross", null].includes(item);
    })
  );
}
...
function positionClick(rowIndex, columnIndex, event) {
  ...
  if (!isValidRowOrColumn(board) || !board.every(isValidColumn)) {
    throw (
      "Expecting 'getBoard' to return a 2d array where all values match are null or one of the strings 'nought' or 'cross'. Actually received: " +
  ...
  if (winner) {
    if (
      ...
      !["noughts", "crosses", "nobody"].includes(winner)
    ) {
      throw (
